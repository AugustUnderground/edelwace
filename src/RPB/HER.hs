...
mkBuffer = Buffer ft ft ft ft bt ft ft
  where
    fopts = T.withDType dataType . T.withDevice cpu $ T.defaultOpts
    bopts = T.withDType T.Float  . T.withDevice cpu $ T.defaultOpts
...
sampleTargets Final _ prd buf@Buffer{..} = pure buf'
  where
    ...
    buf'     = push (2 * bs) prd buf states actions states' tgt' targets'
